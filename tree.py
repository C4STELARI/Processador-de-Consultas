...
def optimize_tree(node):
    if node.type_node == 'Selection' and node.children[0].type_node == 'Join':
        selection_node = node
        join_node = node.children[0]
        condition = selection_node.name
        
        right_table = join_node.children[1]
        left_table = join_node.children[0]
        
        if right_table.name in condition:
            selection_node.children = [right_table]
            join_node.children[1] = selection_node
            return join_node
            
        elif left_table.name in condition:
            selection_node.children = [left_table]
            join_node.children[0] = selection_node
            return join_node

    for i, child in enumerate(node.children):
        node.children[i] = optimize_tree(child)
...
def apply_projection_heuristic(node, required_columns=None):
    ...
    if node.type_node == 'Projection':
        ...
        node.children[0] = apply_projection_heuristic(node.children[0], required_columns)
        return node
    ...
    if node.type_node == 'Join':
        ...
        required_columns.update(potential_cols)
        node.children[0] = apply_projection_heuristic(node.children[0], required_columns)
        node.children[1] = apply_projection_heuristic(node.children[1], required_columns)
        return node
